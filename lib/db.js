...
function buildOrderBy(orderBy) {
  ...
  // Filter out nested relation orderBy (e.g. { question: { order: 'asc' } }) — not supported in raw SQL
  const validEntries = entries.filter(([, dir]) => typeof dir === 'string')
  if (validEntries.length === 0) return sql``
  const parts = validEntries.map(([k, dir]) =>
...
function makeModel(table, sqlInst = sql) {
  return {
    async findUnique({ where, select, include } = {}) {
      const cond = buildWhere(where)
      const rows = await sqlInst`SELECT * FROM ${sqlInst(table)} WHERE ${cond} LIMIT 1`
    ...
    async findFirst({ where, select, include, orderBy } = {}) {
      ...
      const rows = hasWhere
        ? await sqlInst`SELECT * FROM ${sqlInst(table)} WHERE ${cond}${ord} LIMIT 1`
        : await sqlInst`SELECT * FROM ${sqlInst(table)}${ord} LIMIT 1`
      const row = rowToCamel(rows[0])
      if (!row) return null
      if (include) await attachIncludes(table, [row], include)
      return row
    ...
    async findMany({ where, select, include, orderBy, take } = {}) {
      ...
      if (where && Object.keys(where).length > 0) {
        const cond = buildWhere(where)
        rows = take
          ? await sqlInst`SELECT * FROM ${sqlInst(table)} WHERE ${cond}${ord} LIMIT ${take}`
          : await sqlInst`SELECT * FROM ${sqlInst(table)} WHERE ${cond}${ord}`
      } else {
        rows = take
          ? await sqlInst`SELECT * FROM ${sqlInst(table)}${ord} LIMIT ${take}`
          : await sqlInst`SELECT * FROM ${sqlInst(table)}${ord}`
      }
    ...
    async create({ data }) {
      ...
      const rows = await sqlInst`
        INSERT INTO ${sqlInst(table)} ${sqlInst(cols.map((c, i) => ({ [c]: vals[i] })).reduce((a, b) => ({ ...a, ...b })))}
        RETURNING *
      `
    ...
    async update({ where, data }) {
      ...
      for (const [k, v] of Object.entries(data)) {
        updateData[toSnake(k)] = (typeof v === 'object' && v !== null && !(v instanceof Date))
          ? JSON.stringify(v) : v
      }
      const rows = await sqlInst`
        UPDATE ${sqlInst(table)} SET ${sqlInst(updateData)} WHERE ${cond} RETURNING *
      `
      return rowToCamel(rows[0])
    },
    ...
    async delete({ where }) {
      const cond = buildWhere(where)
      const rows = await sqlInst`DELETE FROM ${sqlInst(table)} WHERE ${cond} RETURNING *`
      return rowToCamel(rows[0])
    },
    ...
    async updateMany({ where, data }) {
      ...
      if (where && Object.keys(where).length > 0) {
        const cond = buildWhere(where)
        result = await sqlInst`UPDATE ${sqlInst(table)} SET ${sqlInst(updateData)} WHERE ${cond}`
      } else {
        result = await sqlInst`UPDATE ${sqlInst(table)} SET ${sqlInst(updateData)}`
      }
    ...
    async count({ where } = {}) {
      ...
      if (where && Object.keys(where).length > 0) {
        const cond = buildWhere(where)
        rows = await sqlInst`SELECT COUNT(*) as c FROM ${sqlInst(table)} WHERE ${cond}`
      } else {
        rows = await sqlInst`SELECT COUNT(*) as c FROM ${sqlInst(table)}`
      }
...
const RELATIONS = {
  Teacher: {
    ...
    students: { model: 'Student', fk: 'teacher_id', self: 'id' },
  },
  Test: {
  ...
  Student: {
    teacher: { model: 'Teacher', fk: 'id', self: 'teacher_id', belongsTo: true },
    attendances: { model: 'Attendance', fk: 'student_id', self: 'id' },
  },
  Attendance: {
    student: { model: 'Student', fk: 'id', self: 'student_id', belongsTo: true },
    teacher: { model: 'Teacher', fk: 'id', self: 'teacher_id', belongsTo: true },
  },
}
...
function buildDb(sqlInstance = sql) {
  return {
    ...
    student: makeModelWithSql('Student', sqlInstance),
    attendance: makeModelWithSql('Attendance', sqlInstance),
    enquiry: makeModelWithSql('Enquiry', sqlInstance),
    $transaction,
...
function makeModelWithSql(table, sqlInst) {
  return makeModel(table, sqlInst)
}
